...
pub fn oracle_lookup(board: &Board) -> bool {
	...
	//|| bishop_lone_king(board, Color::White) || bishop_lone_king(board, Color::Black)
}
...
fn knight_lone_king(board: &Board, color: Color) -> bool {
	...
	let enemy_only_king = (board.king(!color).bitboard() ^ enemy_pieces).is_empty();
	...
	enemy_only_king && me_two_or_one_knights && me_only_knights
}
...
fn bishop_lone_king(board: &Board, color: Color) -> bool {
	...
	enemy_only_king && me_only_bishop && me_only_have_bishops
}