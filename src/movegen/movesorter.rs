...
impl MoveSorter {
	...
	pub fn add_material_corrhist(&mut self, boardwrapper: &BoardWrapper, depth: i32, best_alpha: i32, static_eval: i32) {
		...
		*entry = *entry * (1.0 - weight) + ((best_alpha - static_eval) as f32).clamp(-100.0, 100.0) * 280.0 * weight;
	}
	...
	pub fn add_non_pawn_corrhist(&mut self, boardwrapper: &BoardWrapper, depth: i32, best_alpha: i32, static_eval: i32) {
		...
		*entry_white = *entry_white * (1.0 - weight) + ((best_alpha - static_eval) as f32).clamp(-100.0, 100.0) * 280.0 * weight;

		let entry_black = &mut self.non_pawn_corrhist[side_to_move][idx_black];	
		*entry_black = *entry_black * (1.0 - weight) + ((best_alpha - static_eval) as f32).clamp(-100.0, 100.0) * 280.0 * weight;
	}
