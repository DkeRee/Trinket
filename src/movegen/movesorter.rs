...
impl MoveSorter {
	...
	pub fn add_history(&mut self, mv: Move, depth: i32, queued_eval: i32) {
		let history = self.history_table[mv.from as usize][mv.to as usize];
		let change = depth * depth + queued_eval / 150;
	...
	pub fn decay_history(&mut self, mv: Move, depth: i32, queued_eval: i32) {
		let history = self.history_table[mv.from as usize][mv.to as usize];
		let change = depth * depth + queued_eval / 150;
