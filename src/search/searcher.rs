		...
		*/
		...
		for mut sm in legal_moves {
			...
			if moves_searched == 0 {
				...
				let ranks = Rank::Seventh.relative_to(board.side_to_move()).bitboard() | Rank::Sixth.relative_to(board.side_to_move()).bitboard() | Rank::Fifth.relative_to(board.side_to_move()).bitboard();
