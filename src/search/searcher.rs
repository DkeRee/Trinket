		...
		*/
		...
		for mut sm in legal_moves {
			...
			if moves_searched == 0 {
				...
				let mut internal_extension = false;
				...
				//IF ISNT in check
				//IF ISNT extended
				if depth >= Self::HISTORY_DEPTH_MIN && !is_pv && !in_check && moves_searched >= Self::HISTORY_PRUNE_MOVE_LIMIT && !extended && !internal_extension {
