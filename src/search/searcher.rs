		...
		*/
		...
		while legal_index < legal_moves.len() {
				...
				if alpha >= beta {
					...
					if legal_index > 0 {
						for i in 0..(legal_index - 1) {
			    			legal_moves[i as usize].set_conthist(&mut self.movegen.sorter, ply, &boardwrapper.board);
			...
			if staged_movegen && legal_index >= legal_moves.len() {
				staged_movegen = false;
				legal_index = 0; 
			    legal_moves[0].set_conthist(&mut self.movegen.sorter, ply, &boardwrapper.board);
