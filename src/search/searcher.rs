...
impl Searcher<'_> {
	...
	pub fn search(&mut self, abort: &AtomicBool, boardwrapper: &BoardWrapper, mut depth: i32, mut ply: i32, mut alpha: i32, mut beta: i32, past_positions: &mut Vec<u64>, last_move: Option<Move>) -> Option<(Option<Move>, Eval)> {		
		...
		&& ( 
			(tt_hit.as_ref().unwrap().node_kind == NodeKind::Exact) 
		|| (tt_hit.as_ref().unwrap().node_kind == NodeKind::LowerBound && tt_hit.as_ref().unwrap().eval >= beta)
		|| (tt_hit.as_ref().unwrap().node_kind == NodeKind::UpperBound && tt_hit.as_ref().unwrap().eval <= alpha) ) {
