		...
		*/
		...
		for mut sm in legal_moves {
			...
			if moves_searched == 0 {
				...
				//IF IS late move
				//IF is NOT a check
				if !is_pv && depth <= Self::LMP_DEPTH_MAX && sm.movetype == MoveType::Quiet && alpha > -Score::CHECKMATE_DEFINITE && moves_searched > Self::LMP_MULTIPLIER * depth && !in_check && !move_is_check {
					past_positions.pop();
					break;
				}
